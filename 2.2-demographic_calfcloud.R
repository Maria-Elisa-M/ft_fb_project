...
# remove extra spaces on sting data
...
# filter all record for cows born after 2019
dc_demo_data <- dc_demo_data%>%filter(BDAT >= as.Date("2019-01-01"))

# FT DC merging -----

# merge first set of calves
merged1 <- merge(dc_demo_data, all_summary, 
                 by.x = c('ID'), by.y =  c('farm_animal_id'))
...
calf_data <- demo_info%>% select(c("ID", "case_no", "BDAT", "DREG","DID", "gender", "breed"))
